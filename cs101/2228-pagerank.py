...
def compute_ranks(graph):
    d = 0.8 # damping factor: probability of reaching a page even when it has no links
    numloops = 10
    ...
    for i in range(0, numloops):
        newranks = {}
        for page in graph:
            #base: chance that start over from scratch and pick a new random page
            newrank = (1 - d) / npages
            for node in graph: # key
                if page in graph[node]: # page links to this node
                    # inlinks rank; 
                    # d: time when we decide to do a new page 0.8
                    # a page (inlink) with many outlinks will have less influence on the rank
                    newrank = newmark + d * (ranks[node] / len(graph[node]))     
