...
# if a-c is reciprocal? Yes, then do not include a for c's rank
# check if there is a way back from c to a at most k links
# if k = 0, the only links that are excluded are itself
# if k = 2, can we click at most 2 links from c to go back to a...
# if k = 3 and it is reciprocal, we should not include a for any values of k >= 3

def is_recip(graph, source, destination, k):
    if k == 0:
        if destination == source:
            return True
        return False
    if source in graph(destination): # k >= 1
        return True
    for node in graph[destination]:
        # one step
        # check if there is a path from that link to the source of length k-1
        if is_recip(graph, source, node, k-1):
            return True
    return False
        ...
        for page in graph:
            ...
            for node in graph:
                if page in graph[node]:
                    if not is_recip(graph, node, page, k):
